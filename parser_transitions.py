...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            dependency_couple = (self.stack[-1], self.stack[-2])
            self.dependencies.append(dependency_couple)
            self.stack.pop(-2)
        elif transition =="RA":
            dependency_couple = (self.stack[-2], self.stack[-1])
            self.dependencies.append(dependency_couple)
            self.stack.pop(-1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parses = [PartialParse(snt) for snt in sentences]
    unfinished_parses = partial_parses[:]
    while len(unfinished_parses) != 0:
        ...
        for i, transition in enumerate(batch_transitions):
            batch_parses[i].parse_step(transition)
            if batch_parses[i].is_complete():
                unfinished_parses.remove(batch_parses[i])
