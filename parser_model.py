...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Linear(self.n_features * self.embed_size, self.hidden_size)
        self.embed_to_hidden_bias = nn.Linear(self.hidden_size, 1)
        self.hidden_to_logits_weight = nn.Linear(self.hidden_size, self.n_classes)
        self.hidden_to_logits_bias = nn.Linear(self.n_classes, 1)
    ...
    def embedding_lookup(self, w):
        ...
        # batch_emb = torch.empty(batch_size, n_features * embedding_size)
        # W1 = W.unsqueeze(1).repeat(1, embedding_size, 1)
        # for i in range(batch_size):
        #     b_e = torch.gather(emb, 0, W1[i].T)
        #     batch_emb[i, :] = b_e.flatten()
        x = torch.index_select(self.embeddings, w.flatten()).reshape(w.shape[0],-1)
    ...
    def forward(self, w):
        ...
        h = nn.ReLU(torch.matmul(emb, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        if self.training:
            h = self.dropout(h)
